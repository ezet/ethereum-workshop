...
contract Coinit {
    ...
    */
    ...
    function createAccount(string _name, string _email) returns (bool) {
        ...
        accounts[msg.sender] = Account({addr : msg.sender, validated : false, exist : true, balance : 0, name : _name, email : _email});
        return true;
    }
    ...
    function validateAccount(address _accountAddr) isAdmin returns (bool) {
        if (accounts[_accountAddr].validated) return false;
    ...
    function sendCoin(address _receiver, int _amount) returns (bool) {
        require(_amount > 0);
        int new_amount = accounts[msg.sender].balance - _amount;
        if (new_amount < 0) return false;
        accounts[msg.sender].balance = new_amount;
        accounts[_receiver].balance += _amount;
        return true;
    }
    ...
    function createAndGiveMoneyToAllValidatedAccounts(int _amount) isAdmin {
        //        require(_amount > 0);
        for (uint i = 0; i < validatedAccounts.length; i++) {
            accounts[validatedAccounts[i]].balance += _amount;
        }
    }
    ...
    function payOut() isAdmin {
        for (uint i = 0; i < payoutAccounts.length; i++) {
            PayoutAccount payout = payoutAccounts[i];
            accounts[payout.addr].balance -= payout.amount;
        }
    ...
    modifier isAdmin() {
        ...
        // Checking that the sender is admin
        _;
        // Returns the function where the modifier was called and continues into the function
    }
